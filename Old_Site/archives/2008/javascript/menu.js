...
function RePos(){
  ...
  if(MenuCentered=='justify'&&FirstLineHorizontal){
    ...
    for(i=0;i<NoOfFirstLineMenus;i++){P.style.width=W+P_X;P=P.PrvMbr}}
  StaticPos=-1;
...
function Go(){
  ...
  if(WebMasterCheck){ if(!Check('Menu',NoOfFirstLineMenus)){status='build aborted';return}}
  ...
  if(FrstCreat){
    FrstCntnr=CreateMenuStructure('Menu',NoOfFirstLineMenus);
    FrstCreat=AcrssFrms?0:1}
  else CreateMenuStructureAgain('Menu',NoOfFirstLineMenus);
  if(TargetLoc)ClcTrgt();
  if(MenuCentered)ClcLft();
  if(MenuVerticalCentered)ClcTp();
  PosMenu(FrstCntnr,StartTop,StartLeft);
...
function ClcJus(){
  ...
  var Size=Math.round(((FrstWinWdth-a*BorderWidth)/NoOfFirstLineMenus)-b),i,j;
  for(i=1;i<NoOfFirstLineMenus+1;i++){j=eval('Menu'+i);j[5]=Size}
  StartLeft=0}
